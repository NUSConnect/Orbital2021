...
export default function DemoteSuperAdminScreen ({ props, navigation, route }) {
  ...
  const handleDemote = (userId, userName) => {
    Alert.alert(
      ...
      [
        ...
        {
          text: 'Confirm',
          onPress: () => demoteUser(userId, userName)
        }
      ],
      { cancelable: false }
    )
  ...
  const demoteUser = async (userId, userName) => {
    ...
    firebase.firestore().collection('users').doc(userId).update({ superAdmin: false })
    Alert.alert('Success!', 'Successfully demoted ' + userName + ' from SuperAdmin!')
  }
  ...
  const ItemView = ({ item }) => {
    ...
    // Flat List Item
      <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }}>
        ...
        <TouchableOpacity
          style={styles.button}
          onPress={() => handleDemote(item.userId, item.name)}
        >
